    ...
    <div className="min-h-screen bg-slate-900 text-white flex flex-col items-center justify-center p-6 text-center">
      <div className="bg-slate-800 border-2 border-red-500 rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4">
        ...
        <h2 className="text-2xl font-bold text-red-400">
          {t.errorTitle || 'Application Error'}
        </h2>

        <p className="text-slate-300 text-sm">
          {t.errorDefaultMessage || 'An unexpected error occurred. Technical telemetry has been recorded.'}
        </p>
        ...
        <button
          onClick={onReset}
          className="w-full min-h-[56px] bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold rounded-xl flex items-center justify-center gap-2 text-lg shadow-lg active:scale-98 transition cursor-pointer"
        >
          <RefreshCw className="w-6 h-6" />
          {t.errorTryAgain || 'Reload App'}
        </button>
      </div>
    </div>
  );
