      ...
      <div className="p-5 sm:p-6 border-b border-slate-200 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500">
              {t.shift_registrySubtitle || 'Day & Night production telemetry, stitch tallies and machine downtime tracking'}
            </p>
          </div>
          ...
          >
            ...
            <span>{t.shift_logShiftCounter || t.navShiftNew || 'Log Shift Counter'}</span>
          </button>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 pt-1">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-sky-50 border border-sky-200 text-xs text-sky-800">
            ...
            <span>{t.shift_totalOutputChip || 'Total Output:'} <strong className="font-bold text-slate-900">{formatNumber(totalMetersToday)} {t.dash_metersUnit || 'm'}</strong></span>
          </span>
          ...
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-50 border border-blue-200 text-xs text-blue-800">
            ...
            <span>{t.shift_totalStitchesChip || 'Total Stitches:'} <strong className="font-bold text-slate-900">{formatNumber(totalStitchesToday)} {t.dash_stitchesUnit || 'st.'}</strong></span>
          </span>
          ...
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-rose-50 border border-rose-200 text-xs text-rose-800">
            ...
            <span>{t.shift_totalDowntimeChip || 'Total Downtime:'} <strong className="font-bold text-rose-700">{totalDowntimeMinutes} min</strong></span>
          </span>
        </div>
      </div>
      ...
      <div className="p-5 sm:p-6 space-y-4">
        ...
        {downtimeStats.length > 0 && (
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-2">
            <div className="flex items-center gap-1.5 text-xs font-bold text-slate-700 uppercase tracking-wider">
              ...
              <span>{t.shift_downtimeAnalytics || 'Downtime Analytics'}</span>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-1">
              {downtimeStats.map((d, idx) => (
                <div key={idx} className="bg-white border border-slate-200 p-2.5 rounded-lg space-y-0.5">
                  ...
                  <div className="text-2xs text-slate-400">{d.incident_count} {t.shift_incidents || 'incidents'}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 bg-slate-50 p-2 rounded-xl border border-slate-200">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder={t.shift_searchPlaceholder || 'Search by machine number, karigar name or design code...'}
              value={searchTerm}
          ...
          <div className="flex items-center gap-1 bg-white p-1 rounded-lg border border-slate-200 text-xs">
            {[
              { key: 'ALL', label: t.shift_filterAll || 'ALL' },
              { key: 'DAY', label: t.shift_filterDay || 'DAY' },
              { key: 'NIGHT', label: t.shift_filterNight || 'NIGHT' },
            ].map(({ key, label }) => (
              <button
                key={key}
                onClick={() => setShiftFilter(key)}
                className={`px-3 py-1 rounded text-2xs font-medium transition cursor-pointer ${
                  shiftFilter === key
          ...
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="bg-slate-50 text-slate-500 font-semibold border-b border-slate-200">
                <th className="p-3.5">{t.shift_thDateShift || 'Date & Shift'}</th>
                <th className="p-3.5">{t.shift_thMachine || 'Machine'}</th>
                <th className="p-3.5">{t.shift_thKarigar || 'Karigar'}</th>
                <th className="p-3.5 text-right">{t.shift_thCounter || 'Counter (Start ➔ End)'}</th>
                <th className="p-3.5 text-right">{t.shift_thStitches || 'Total Stitches'}</th>
                <th className="p-3.5 text-right">{t.shift_thMeters || 'Meters Output'}</th>
                <th className="p-3.5 text-center">{t.shift_thDowntime || 'Downtime'}</th>
              </tr>
            </thead>
              ...
              {filtered.map((s) => (
                <tr key={s.id} className="hover:bg-slate-50/80 transition">
                  ...
                  <td className="p-3.5 font-semibold text-slate-900">
                    {t.dash_machinePrefix || 'Machine'} #{s.machine?.machine_no || '1'}
                    <span className="text-2xs text-slate-400 font-normal block font-mono">
                      {s.machine?.head_count || 32} {t.shift_headsUnit || 'Heads'}
                    </span>
                  </td>
                  <td className="p-3.5">
                    <div className="font-medium text-slate-900">{s.karigar?.name || 'Operator'}</div>
                    <div className="text-2xs text-slate-400">{s.design_no || t.shift_standardPattern || 'Standard Pattern'}</div>
                  </td>
                  ...
                  <td className="p-3.5 text-right font-mono font-semibold text-slate-900">
                    {formatNumber(s.total_stitches)} {t.dash_stitchesUnit || 'st.'}
                  </td>
                  <td className="p-3.5 text-right font-mono font-bold text-slate-900">
                    {formatNumber(s.total_meters)} {t.dash_metersUnit || 'm'}
                  </td>
                  <td className="p-3.5 text-center">
                    {s.downtime_minutes > 0 ? (
                      <span className="px-2 py-0.5 rounded text-2xs font-mono font-medium bg-rose-50 text-rose-700 border border-rose-200">
                        {s.downtime_minutes}m ({s.downtime_reason || t.shift_stopReason || 'Stop'})
                      </span>
                    ) : (
              ...
              {filtered.length === 0 && !loading && (
                <tr>
                  <td colSpan={7} className="p-8 text-center text-slate-400">
                    {t.shift_noLogs || 'No shift logs found. Click "+ Log Shift Counter" to add a new shift log.'}
                  </td>
                </tr>
              )}
            </tbody>
