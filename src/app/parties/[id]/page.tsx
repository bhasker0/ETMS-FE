            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-2xs font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1">
                  ...
                  વેપારી ખાતાવહી • Party Ledger Statement
                </span>
                <span
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-600 mt-1.5">
                <span className="font-mono bg-slate-100 px-2 py-0.5 rounded text-2xs text-slate-700 border border-slate-200">
                  GSTIN: {party.gstin || 'Unregistered / URP'}
                </span>
                {party.mobile && (
                ...
                <span className="flex items-center gap-1 text-slate-500">
                  ...
                  Credit: {party.credit_period_days || 15} Days
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs space-y-2">
          <div className="flex items-center justify-between text-slate-500">
            <span className="text-2xs font-bold uppercase tracking-wider">Total SAC 9988 Billed</span>
          ...
          <div className="text-2xs text-slate-500">
            {metrics.total_invoices_count} invoices generated • {formatNumber(metrics.total_outward_meters)} meters
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs space-y-2">
          <div className="flex items-center justify-between text-slate-500">
            <span className="text-2xs font-bold uppercase tracking-wider">Outstanding Balance</span>
          ...
          <div className="text-2xs text-slate-500">
            Includes opening balance of {formatINR(metrics.opening_balance)}
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs space-y-2">
          <div className="flex items-center justify-between text-slate-500">
            <span className="text-2xs font-bold uppercase tracking-wider">Total Inward Fabric</span>
          ...
          <div className="text-2xs text-slate-500">
            {metrics.total_inward_lots} Delivery Challans registered
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs space-y-2">
          <div className="flex items-center justify-between text-slate-500">
            <span className="text-2xs font-bold uppercase tracking-wider">Fabric In Process</span>
          ...
          <div className="text-2xs text-slate-500">
            Pending jobwork embroidery delivery
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
            ...
            <span>Receivable Aging Breakdown (ચુકવણી મુદત વિશ્લેષણ)</span>
          </h2>
          <span className="text-2xs text-slate-500">Credit Term: {party.credit_period_days || 15} days</span>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="p-3.5 bg-emerald-50 border border-emerald-200 rounded-xl space-y-1">
            <div className="text-2xs font-bold uppercase text-emerald-800">Current (&lt;15 Days)</div>
            <div className="text-lg font-bold text-emerald-900 font-mono">
              {formatINR(metrics.aging.within_15_days)}
            </div>
            <div className="text-2xs text-emerald-700">Within standard credit cycle</div>
          </div>

          <div className="p-3.5 bg-amber-50 border border-amber-200 rounded-xl space-y-1">
            <div className="text-2xs font-bold uppercase text-amber-800">Due (16 - 30 Days)</div>
            <div className="text-lg font-bold text-amber-900 font-mono">
              {formatINR(metrics.aging.days_16_to_30)}
            </div>
            <div className="text-2xs text-amber-700">Follow-up recommended</div>
          </div>

          <div className="p-3.5 bg-rose-50 border border-rose-200 rounded-xl space-y-1">
            <div className="text-2xs font-bold uppercase text-rose-800">Overdue (&gt;30 Days)</div>
            <div className="text-lg font-bold text-rose-900 font-mono">
              {formatINR(metrics.aging.above_30_days)}
            </div>
            <div className="text-2xs text-rose-700">Critical overdue balance</div>
          </div>
        </div>
      </div>
        ...
        <div className="p-5 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-slate-900">
              Chronological Ledger Entries ({timeline.length})
            </h2>
          </div>
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-2xs uppercase tracking-wider text-slate-500 font-semibold">
                  ...
                  <th className="p-3.5 text-right">Debit (Invoiced ₹)</th>
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700">
                ...
                <tr className="bg-slate-50/50 font-semibold">
                  ...
                  <td className="p-3.5 font-mono text-slate-500">—</td>
                  <td className="p-3.5">Opening Master Balance</td>
